...
/**
 * ClinicPro - نظام حجز المواعيد
 * يحتوي على جميع الدوال الخاصة بحجز المواعيد
 * @version 1.0.0
...
class BookingSystem {
     ...
     */
    async init() {
        console.log('🚀 بدء تشغيل نظام الحجز...');
        ...
        try {
            ...
            // تحميل الحجز السريع (إذا كان المستخدم مسجل الدخول)
            await this.setupQuickBooking();
            
            // تحديث حالة الأزرار
            this.updateStepButtons();
            
            console.log('✅ تم تهيئة نظام الحجز بنجاح');
            
        } catch (error) {
            console.error('❌ فشل في تهيئة نظام الحجز:', error);
            this.showError('فشل في تحميل نظام الحجز', error.message);
        }
    }
    ...
    setupWizardSteps() {
        ...
        // تحديث الخطوات عند تغيير حجم النافذة
    ...
    goToStep(stepNumber) {
        ...
        // التحقق من استكمال الخطوات السابقة
        if (!this.validateStep(this.currentStep)) {
            this.showToast('يرجى استكمال جميع الحقول المطلوبة', 'warning');
            return;
        }
    ...
    saveStepData(stepNumber) {
        switch (stepNumber) {
            case 1: // حفظ بيانات الطبيب
                this.saveDoctorData();
                break;
                
            case 2: // حفظ بيانات التاريخ
                this.saveDateData();
                break;
                
            case 3: // حفظ بيانات الوقت والنوع
                this.saveTimeAndTypeData();
                break;
                
            case 4: // حفظ بيانات المريض
    ...
    savePatientData() {
        ...
        // دمج بيانات المريض مع بيانات الحجز
    ...
    updateStepButtons() {
        ...
        nextButtons.forEach(button => {
            ...
            if (nextStep === currentStep + 1) {
                // التحقق مما إذا كان الزر يجب أن يكون معطلاً
    ...
    /**
     * تحديث عرض اختيار الطبيب
     */
    updateDoctorSelection() {
        // تحديث معلومات الطبيب المختار في الخطوة 2
    ...
    /**
     * تحديث عرض اختيار التاريخ
     */
    updateDateSelection() {
        // تحديث التقويم مع تواريخ الطبيب المختار
        if (this.selectedDoctor) {
    ...
    /**
     * تحديث عرض اختيار الوقت
     */
    updateTimeSelection() {
        if (this.selectedDoctor && this.selectedDate) {
            // تحديث معلومات التاريخ المختار
    ...
    updatePatientForm() {
        if (this.selectedDoctor && this.selectedDate && this.selectedTime && this.selectedType) {
            ...
            // ملء نموذج المريض إذا كان مسجلاً
    ...
    updateConfirmation() {
        if (this.bookingData) {
            // تحديث جميع المعلومات في صفحة التأكيد
    ...
    async loadDoctors() {
        try {
            ...
            // الحصول على الأطباء من قاعدة البيانات
            ...
            if (doctors.length === 0) {
                doctorsContainer.innerHTML = `
                    <div class="error-state">
                        ...
                        <h3>لا توجد أطباء متاحين</h3>
                        <p>لا توجد أطباء متاحين للحجز في الوقت الحالي</p>
                    </div>
                `;
                return;
            }
            ...
            if (doctorsContainer) {
                doctorsContainer.innerHTML = `
                    <div class="error-state">
                        ...
                        <h3>خطأ في تحميل الأطباء</h3>
                        <p>حدث خطأ أثناء تحميل قائمة الأطباء</p>
    ...
    createDoctorCard(doctor) {
        ...
        const statusText = isAvailable ? 'متاح' : 'مشغول';
        
        // تحديد أيام العمل
        const scheduleDays = doctor.schedule?.map(s => s.day).join('، ') || 'غير محدد';
    ...
    selectDoctor(doctor, cardElement) {
        ...
        // تحديث حالة الزر التالي
        this.updateStepButtons();
        
        // إظهار رسالة تأكيد
        this.showToast(`تم اختيار الدكتور ${doctor.name}`, 'success');
    }
    ...
    updateSelectedDoctorInfo() {
        ...
        container.innerHTML = `
            <h4><i class="fas fa-user-md"></i> الطبيب المختار</h4>
            ...
            <div class="doctor-detail">
                ...
                <span>${this.selectedDoctor.experience || 'خبرة'}</span>
            </div>
            <div class="doctor-detail">
                <i class="fas fa-money-bill-wave"></i>
    ...
    loadAppointmentTypes() {
        ...
        const appointmentTypes = this.config?.get('APPOINTMENT_TYPES') || [
            { id: 'checkup', name: 'كشف جديد', duration: 30, price: 0 },
            { id: 'followup', name: 'متابعة', duration: 20, price: 0 },
            { id: 'consultation', name: 'استشارة', duration: 45, price: 0 },
            { id: 'emergency', name: 'طوارئ', duration: 60, price: 50 },
            { id: 'surgery', name: 'جراحة', duration: 120, price: 100 }
        ];
        ...
        appointmentTypes.forEach(type => {
            ...
            option.innerHTML = `
                ...
                <div class="type-duration">${type.duration} دقيقة</div>
                <div class="type-price">${type.price > 0 ? `+ ${this.formatCurrency(type.price)}` : 'مشمول'}</div>
            `;
    ...
    renderCalendarDays() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (date < today) {
                ...
                // التحقق من توفر التاريخ للطبيب المختار
                if (this.selectedDoctor) {
            ...
            // التحقق مما إذا كان التاريخ هو المختار
        ...
        // تحديث أيام العمل المتاحة للطبيب
        this.updateAvailableDays();
    }

    /**
     * التحقق من توفر التاريخ للطبيب
     */
    isDateAvailableForDoctor(date) {
        ...
        // التحقق من أن الطبيب يعمل في هذا اليوم
        if (!doctorSchedule || !doctorSchedule.isAvailable) {
            return false;
        }
        
        // التحقق من أن التاريخ ليس في عطلة
    ...
    updateAvailableDays() {
        ...
        availableDays.forEach(day => {
            ...
            dayElement.addEventListener('click', () => {
                // البحث عن أقرب تاريخ لهذا اليوم
    ...
    selectDayByName(dayName) {
        ...
        // البحث عن أقرب تاريخ في المستقبل لهذا اليوم
        for (let i = 0; i < 30; i++) { // البحث لمدة شهر
            const date = new Date();
    ...
    selectDate(date, element) {
        // إلغاء تحديد التاريخ السابق
        const previouslySelected = document.querySelector('.calendar-day.selected');
        if (previouslySelected) {
            previouslySelected.classList.remove('selected');
        }
        
        // إلغاء تحديد أيام العمل السابقة
        ...
        this.showToast(`تم اختيار التاريخ: ${formattedDate}`, 'success');
    }

    /**
     * تحديث التقويم بجدول الطبيب
     */
    ...
    async loadAvailableTimeSlots() {
        ...
        try {
            ...
            container.innerHTML = `
                <div class="error-state">
                    <i class="fas fa-exclamation-triangle"></i>
                    <h3>خطأ في تحميل الأوقات</h3>
                    <p>حدث خطأ أثناء تحميل الأوقات المتاحة</p>
                </div>
            `;
        }
    }
    ...
    generateTimeSlots(startTime, endTime, doctorSchedule, appointments) {
        ...
        // إنشاء قائمة بالمواعيد المحجوزة
        ...
        for (let time = Math.max(startMinutes, scheduleStart); 
            ...
            // التحقق من عدم وجود موعد محجوز في هذا الوقت
    ...
    selectTime(time, element) {
        ...
        // إظهار رسالة تأكيد
        this.showToast(`تم اختيار الوقت: ${time}`, 'success');
    }
    ...
    /**
     * ملء نموذج المريض تلقائياً
     */
    prefillPatientForm() {
        // إذا كان المستخدم مسجلاً الدخول، يمكن ملء البيانات تلقائياً
    ...
    async confirmBooking() {
        try {
            ...
            if (result.success) {
                ...
                // إخفاء زر التأكيد وإظهار زر الحجز الجديد
                const confirmButton = document.getElementById('confirm-booking');
                const newBookingButton = document.getElementById('new-booking');
                ...
                throw new Error(result.message || 'فشل في حجز الموعد');
            }
            
        } catch (error) {
            console.error('خطأ في تأكيد الحجز:', error);
            this.showToast('فشل في تأكيد الحجز: ' + error.message, 'error');
        }
    }
    ...
    async savePatientToDatabase() {
        try {
            // البحث عن المريض إذا كان موجوداً
            ...
            // لا نوقف العملية لأن حفظ المريض ليس ضرورياً للحجز
        }
    }
    ...
    showBookingSuccess(bookingData) {
        ...
        const successHTML = `
            <div class="booking-success">
                ...
                <p>
                    تم تأكيد حجز موعدك مع ${bookingData.doctorName}<br>
                    التاريخ: ${bookingData.date} | الوقت: ${bookingData.time}<br>
                    رقم الحجز: <strong>${bookingData.code}</strong>
                </p>
                ...
                <p class="mt-4 text-muted">
                    ستصلك رسالة تأكيد على هاتفك المحمول قبل الموعد بـ 24 ساعة
                </p>
            </div>
        `;
        
        // استبدال محتوى خطوة التأكيد
    ...
    resetBooking() {
        ...
        // إظهار رسالة
        this.showToast('تم بدء عملية حجز جديدة', 'info');
    }
    ...
    async setupQuickBooking() {
        ...
        if (currentUser) {
            ...
            // تحميل الأطباء للحجز السريع
            await this.loadQuickBookingDoctors();
            
            // إعداد أحداث الحجز السريع
    ...
    async loadQuickBookingDoctors() {
        ...
        try {
            ...
            console.error('خطأ في تحميل الأطباء للحجز السريع:', error);
        }
    }
    ...
    async loadQuickBookingTimes() {
        ...
        try {
            ...
            console.error('خطأ في تحميل الأوقات للحجز السريع:', error);
        }
    }
    ...
    async processQuickBooking() {
        ...
        if (!doctorId || !date || !time) {
            this.showToast('يرجى اختيار الطبيب والتاريخ والوقت', 'warning');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                this.showToast('تم الحجز السريع بنجاح!', 'success');
            ...
            console.error('خطأ في الحجز السريع:', error);
            this.showToast('فشل في الحجز السريع: ' + error.message, 'error');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        console.log('✅ نظام الحجز جاهز للعمل');
    }, 1000);
});
...
// نهاية الملف
console.log('✅ تم تحميل booking.js بنجاح');
